...
const event_source_parse_js_1 = require("./event-source-parse.cjs");
...
async function getResponse(request, config) {
    ...
    if (stageOne.status >= 200 && stageOne.status !== 204) {
        if (config.responseType === "stream") {
            ...
            if (!contentType?.startsWith(event_source_parse_js_1.EventStreamContentType)) {
                ...
                throw new Error(`Expected content-type to be ${event_source_parse_js_1.EventStreamContentType}, Actual: ${contentType}`);
            }
            await (0, event_source_parse_js_1.getBytes)(stageOne.body, (0, event_source_parse_js_1.getLines)((0, event_source_parse_js_1.getMessages)(config.onmessage)));
        }
        else {
